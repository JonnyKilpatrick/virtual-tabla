...
MapLeapPointsToScreen mapToScreen; // Class to map the real co-ordinates to screen space
...
void setup() 
{
  ...
  mapToScreen = 
    new MapLeapPointsToScreen(leapMinimum, leapMaximum, screenMinimum, screenMaximum);
...
void draw() 
{
  ...
  // Larger Drum
  fill(247, 232, 179);
  ellipse(width * (3.5/ 12.0), height/2.0, width * (5.0/12.0), width * (5.0/12.0));
//  // Syahi in middle
//  fill(32, 24, 18);
//  ellipse(width * (3.5/ 12.0), height/2.0, width * (1.0 /8.0), width * (1.0/8.0));
  ...
  // Smaller Drum
  fill(247, 232, 179);
  ellipse(width * (9.0/12.0), height/2.0, width/3.0, width/3.0);
//  // Syahi in middle
//  fill(32, 24, 18);
//  ellipse(width * (9.0/12.0), height/2.0, width * (1.0/8.0), width * (1.0/8.0));
  ...
  for (Hand hand : frame.hands()) 
  {
    ...
    for (Finger finger : hand.fingers ()) 
    {
      ...
      boolean sound = gestureRecogniser.checkForGestures(finger);
